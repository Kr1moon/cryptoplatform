...
const WS_BASE = `${process.env.NEXT_PUBLIC_COINGECKO_WS_BASE_URL}?x_cg_pro_api-key=${process.env.NEXT_PUBLIC_COINGECKO_API_KEY}`;
...
export const useCoinGeckoWebSocket = ({ 
    ...
    useEffect(() => {
        ...
        const send = (paylod: Record<string, unknown>) =>
           ws.send(JSON.stringify(paylod)); 
        ...
        const handleMessage = (event: MessageEvent) => {
            ...
            if(msg.type === 'confirm_subscription') {
                    const { channel } = JSON.parse(msg?. identifier ?? '');                    subscribed.current.add(channel);
                }       
    ...
    useEffect(() => {
        ...
        const subscribe = (chanel: string, data?: Record<string, unknown>) => {
           if (subscribed.current.has(chanel)) return;
             ...
             send ({ command: 'subscribe', identifier: JSON.stringify({ chanel })
            })
            if(data) {
                send({command: 'message',
                    idenifier: JSON.stringify({ chanel }),
